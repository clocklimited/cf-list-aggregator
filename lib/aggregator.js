...
function createAggregator(crudService, options) {
  ...
  function aggregateList(list, cb) {
    ...
    crudService.findPublic(q.query, q.options, function (err, unprocessedResults) {
      ...
      var articles = list.type === 'manual' ? list.articles : null
      processResults(unprocessedResults, articles, function (error, results) {
        ...
        if (list.type !== 'manual') {
          ...
          var customListItems = getCustomItemOrder(results, articles, options.date)
